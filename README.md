...
| MineStudio minimal workflow smoke test | install/import/module/simulator check | Ran clean package import, small dependency blocker-reduction, module inventory, and simulator readiness checks on AutoDL. | `cv2` blocker resolved; simulator import now blocked by `torch` and display/runtime support | `docs/minestudio_smoke_test.md`, `experiments/minestudio-smoke-test/` |
